                    ...
                    const Image<byte>& im2, int i2, int j2, float m2)
{
    float dist=0.0f;
    // ------------- TODO -------------
    float v1=0.0f, v2=0.0f, cov = 0.0f;
    for (int ri=-win; ri<=win; ri++)
        for (int rj=-win; rj<=win; rj++) {
            cov += (im1(i1+ri,j1+rj)-m1)*(im2(i2+ri,j2+rj)-m2);
            v1 += (im1(i1+ri,j1+rj)-m1)*(im1(i1+ri,j1+rj)-m1);
            v2 += (im2(i2+ri,j2+rj)-m2)*(im2(i2+ri,j2+rj)-m2);
        }
    dist = cov/sqrt(v1)/sqrt(v2);
    return dist;
}
...
static float sum(const Image<byte>& im, int i, int j)
{
    ...
    for (int ri=-win; ri<=win; ri++)
        for (int rj=-win; rj<=win; rj++) {
            if ((i+ri>=0) & (i+ri<im.width()) & (j+rj>=0) & (j+rj<=im.height()))
                s += im(clip(i+ri, 0, im.width()), clip(j+rj, 0, im.height()));
        }
    return s;
}
